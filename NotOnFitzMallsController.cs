...
namespace WebApplication6
{
    public class NotOnFitzMallsController : Controller
    {
        ...
        public ActionResult DrillDown(string StoreBranch, string Location, string Make, string StatusCode, string OnFitzMall)
        {
            ...
            string search_Status = "1 2 4 9 11 12 14";

            System.Diagnostics.Debug.WriteLine("NotOnFitzMallsController- Getting View: Make:" + Make + " Location:" + Location + " Store/Branch:" + StoreBranch + " Status: " + parStatusCode + " NEW");

            // is this a on-fitzmall drilldown or what should be on fitzmall?
            if (OnFitzMall == "1")
            {
                return RedirectToAction("DrillDown", "Inventories", new { Make = Make, StoreBranch = StoreBranch, StatusCode = parStatusCode, Location = Location });
            }
            else
            {
                if (Make + "" == "")
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine(".Where(d => d.STAT_CODE == parStatusCode && d.NEW_USED == N");
                        return View(db.NotOnFitzMalls.ToList().Where(d => d.STAT_CODE == parStatusCode && d.NEW_USED == "N" && ((d.STAT_CODE == 1) || (d.STAT_CODE == 2) || (d.STAT_CODE == 4) || (d.STAT_CODE == 9) || (d.STAT_CODE == 12) || (d.STAT_CODE == 14))));
                    }
                }
                else
                {
                    if (StoreBranch == "")
                    {
                        ...
                        if (parStatusCode == 0)
                        {
                            System.Diagnostics.Debug.WriteLine("d => d.MAKE == Make && d.LOCATION == Location && d.STORE_BRANCH == StoreBranch && d.NEW_USED");
                            return View(db.NotOnFitzMalls.ToList().Where(d => d.MAKE == Make && d.LOCATION == Location && d.STORE_BRANCH == StoreBranch && d.NEW_USED == "N" && ((d.STAT_CODE == 1) || (d.STAT_CODE == 2) || (d.STAT_CODE == 4) || (d.STAT_CODE == 9) || (d.STAT_CODE == 12) || (d.STAT_CODE == 14))));
                        }
                        else
                        {
                            System.Diagnostics.Debug.WriteLine("Where(d => d.MAKE == Make && d.LOCATION == Location && d.STORE_BRANCH == StoreBranch && d.STAT_CODE == parStatusCode && d.NEW_USED == N");
                            return View(db.NotOnFitzMalls.ToList().Where(d => d.MAKE == Make && d.LOCATION == Location && d.STORE_BRANCH == StoreBranch && d.STAT_CODE == parStatusCode && d.NEW_USED == "N"));
                        }
                    }
                }
